...
function table_name()
{
	...
	$result=mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die("error in function table_name()".mysqli_error($GLOBALS["___mysqli_ston"]));
	$row = mysqli_fetch_array($result);
	if(!$row)
	die("error in function table_name() output". mysqli_error($GLOBALS["___mysqli_ston"]));
	else
	return $row[0];
}
...
function column_name($idee)
{
	...
	$result=mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die("error in function column_name()".mysqli_error($GLOBALS["___mysqli_ston"]));
	$row = mysqli_fetch_array($result);
	if(!$row)
	die("error in function column_name() result". mysqli_error($GLOBALS["___mysqli_ston"]));
	else
	return $row[0];
}
...
function next_tryy()
{
	...
	mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die("error in function next_tryy()". mysqli_error($GLOBALS["___mysqli_ston"]));
}
...
function view_attempts()
{
	...
	$row = mysqli_fetch_array($result);
	if(!$row)
	die("error in function view_attempts()". mysqli_error($GLOBALS["___mysqli_ston"]));
	else
	return $row[0];	
}
